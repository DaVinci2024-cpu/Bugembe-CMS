...
Location: Bugembe, Jinja City, Uganda
Headteacher: Sheikh Yusuf Mutyaba
...
=== STATISTICS ===
${trustStatistics.map((s) => `- ${s.label}: ${s.value}${s.suffix} (${s.description})`).join("\n")}
...
Admissions process:
${admissionsDetails.process.map((p) => `Step ${p.step}: ${p.title} - ${p.description}`).join("\n")}

General requirements:
${admissionsDetails.requirements.general.map((r) => `- ${r}`).join("\n")}

Boarding requirements checklist:
${admissionsDetails.requirements.boardingList.map((r) => `- ${r}`).join("\n")}

=== FREQUENTLY ASKED QUESTIONS (FAQ) ===
${admissionsDetails.faqs.map((f) => `Q: ${f.question}\nA: ${f.answer}`).join("\n\n")}

=== LATEST NEWS ===
${newsArticles.map((a) => `- ${a.title} (${a.category}, ${a.date}): ${a.excerpt}`).join("\n")}
